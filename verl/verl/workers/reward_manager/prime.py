...
#         print(f"[Error] Task failed: {e}, completion: {completion[:200]}")
...
async def single_compute_score(evaluation_func, completion, reference, task, task_extra_info, executor, timeout=300.0):
    ...
    try:
        ...
        future = loop.run_in_executor(
            executor, 
            evaluation_func, 
            task, completion, reference, task_extra_info
        )
        return await asyncio.wait_for(future, timeout=timeout)
    except asyncio.TimeoutError:
        print(f"[Timeout] Task timeout: {completion}")
...
# 使用线程池
executor = ThreadPoolExecutor(max_workers=4)  # 而不是 ProcessPoolExecutor
